...
def global_approval(did, rule_attributes, session):
    ...
    if has_account_attribute(account, 'rule_blacklisted', session=session):
        return False
    ...
    if rule_attributes['lifetime'] > 2592000:
        return False

    # Check if the RSE expression is allowed for the user
    # rse_expression should be of the form "data_brokering_rse_set=<number>"
    other_valid_rse_expressions = ['cms_type=real&rse_type=DISK']
    rse_expression = rule_attributes['rse_expression']
    if not re.match(r'^data_brokering_rse_set=\d+$', rse_expression) and rse_expression not in other_valid_rse_expressions:
        return False
